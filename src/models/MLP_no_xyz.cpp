#include "models/MLP_no_xyz.h"

MLP_no_xyz::MLP_no_xyz(std::string name, int dim, int c_dim, int hidden_size, int n_blocks, bool leak, std::string sample_mode, bool color, std::vector<int> skips, float grid_len, std::string pose_emb, bool concat_feat):
name(name),
color(color),
c_dim(c_dim),
grid_len(grid_len),
