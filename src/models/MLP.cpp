...
MLP::MLP(std::string name, int dim, int c_dim, int hidden_size, int n_blocks, bool color, std::vector<int> skips, float grid_len, std::string pose_emb, bool concat_feat):
name(name),
color(color),
c_dim(c_dim),
grid_len(grid_len),
...
MLP_no_xyz::MLP_no_xyz(std::string name, int dim, int c_dim, int hidden_size, int n_blocks, bool color, std::vector<int> skips, float grid_len):
name(name),
color(color),
c_dim(c_dim),
grid_len(grid_len),
...
torch::Tensor MLP_no_xyz::sample_grid_feature(torch::Tensor p, torch::Tensor grid_feature)
{
	Eigen::MatrixXf bound(3,2);
	bound<<-4.5, 3.82,
		   -1.5, 2.02,
...
torch::nn::Linear MLP_no_xyz::reset_parameters_dense(torch::nn::Linear layer, std::string activation) 
{
	torch::NoGradGuard noGrad;
	if (activation == "relu")
		torch::nn::init::xavier_uniform_(layer->weight, torch::nn::init::calculate_gain(torch::enumtype::kReLU()));
...
torch::Tensor MLP_no_xyz::forward(torch::Tensor p, std::map<std::string, torch::Tensor> c_grid)
{
	auto c = sample_grid_feature(p, c_grid["grid_"+name]).transpose(1, 2).squeeze(0);
	auto h = c;
	int i = 0;
